...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      // ============================================
      // 【无 Timeout/Watchdog 最小化初始化】V260112.01
      // ============================================
      const BUILD_FINGERPRINT = 'V260112.01_no_timeout';
      console.log('[INIT_MINIMAL] 开始初始化 (无 timeout/watchdog)');
      ...
      const mark = (stage: string) => {
        ...
        console.log(`[INIT_MINIMAL] ${stage} +${elapsed}ms`);
      };
      ...
      try {
        ...
        // ============================================
        // 1️⃣ 直接调用 getMedicationsFromCloud (无 timeout 包装)
        // ============================================
        console.log('[INIT_MINIMAL] 调用 getMedicationsFromCloud...');
        mark('fetch medications - start');
        const rawMeds = await getMedicationsFromCloud();
        console.log(`[INIT_MINIMAL] 获取到 ${rawMeds.length} 个药品`);
        mark(`fetch medications - done: ${rawMeds.length} items`);
        ...
        // 3️⃣ 无条件设置 state
        // ============================================
        console.log('[INIT_MINIMAL] 设置 medications state:', medsUI.length);
        safeSetMedications(medsUI, 'init-minimal');
        mark('build UI state - done');

        logger.log(`✅ [初始化] medications 加载完成: ${rawMeds.length} 个药品`);
        ...
        // ============================================
        // 5️⃣ 失败时设置空数组,确保 state 初始化
        // ============================================
        console.error('[INIT_MINIMAL] 初始化失败,设置空数组:', error);
        safeSetMedications([], 'init-minimal-error');
        mark('error');
      } finally {
        // ============================================
        // 6️⃣ 无论成功失败,都结束 loading
        // ============================================
        console.log('[INIT_MINIMAL] 结束 loading');
        setInitialLoading(false);
        setAppInitialized(true);
        isInitializingRef.current = false;
        const totalTime = Math.round(performance.now() - t0);
        console.log(`[INIT_MINIMAL] 初始化完成,总耗时 ${totalTime}ms`);
        logger.log(`✅ [初始化] 完成,耗时 ${totalTime}ms`);
      }
    };
    ...
    const loadLogsInBackground = async () => {
      ...
      try {
        ...
        console.log('[LOGS] fetched:', recentLogs.length);
