...
export async function initRealtimeSync(callbacks: RealtimeCallbacks): Promise<() => void> {
  try {
    ...
    if (!userId) {
      logger.warn('[Realtime] 未登录，跳过 Realtime 初始化');
      return () => {};
    }
    
    const deviceId = getDeviceId();
    logger.log('[Realtime] 初始化同步服务', { userId, deviceId });
    ...
    realtimeChannel = supabase
      ...
      .on('postgres_changes', {
        ...
        logger.log('[Realtime] 服药记录变更（其他设备）', {
          eventType: payload.eventType,
          logId: newData?.id,
          deviceId: newData?.device_id
        });
      ...
      .on('postgres_changes', {
        ...
        logger.log('[Realtime] 用户设置变更（其他设备）', {
          eventType: payload.eventType,
          deviceId: newData?.device_id
        });
