  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42703' || error.message?.includes('does not exist')) {
        ...
        console.log('ℹ️ 版本检查跳过：required_version 列不存在（数据库未迁移），已禁用后续查询');        return; // 静默返回，不报错，不触发刷新
      }
      
      // 其他错误仍然记录（但不阻塞）
      console.warn('⚠️ 版本检查查询失败（非阻塞）:', error.code, error.message);      return; // 静默返回，不阻塞应用启动
    }
