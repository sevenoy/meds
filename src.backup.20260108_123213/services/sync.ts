...
export async function fixLegacyDeviceIds(): Promise<void> {
  ...
  if (!userId) {
    console.log('❌ [fixLegacyDeviceIds] 无 userId，跳过修复');    return;
  }
  ...
  await runWithRemoteFlag(async () => {    try {
      ...
      const { data, error } = await supabase!
        ...
        .select();      
      if (error) {
        console.error('❌ 修复药品 device_id 失败:', error);      } else {
  ...
  try {
    ...
    if (localMeds.length > 0) {
      const medsToSync = localMeds.map(med => {
        ...
        if (medsToSync.length > 0) {          // 批量upsert（Supabase会自动处理insert/update）
    ...
    // 拉取云端medications（批量）    const { data: cloudMeds } = await supabase!
      .from('medications')
      .select('*')
      .eq('user_id', userId);    
    if (cloudMeds && cloudMeds.length > 0) {
      ...
      if (newMeds.length > 0) {
        ...
        console.log(`📦 准备批量添加 ${medsToAdd.length} 条药品到本地:`, medsToAdd.map(m => ({ id: m.id, name: m.name })));        await db.medications.bulkPut(medsToAdd);
        console.log(`✅ 批量添加 ${medsToAdd.length} 条云端药品到本地`);      } else {
        console.log('ℹ️ 所有云端药品都已存在于本地，无需添加');      }
    } else {
      console.log('⚠️ 从Supabase拉取的medications为空或null');    }
    
    console.log('✅ Medications同步完成');  } catch (error) {
