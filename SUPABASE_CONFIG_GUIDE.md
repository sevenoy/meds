...
1. **环境变量**（`.env` 文件，仅开发环境）
2. **localStorage**（浏览器本地存储，用于自定义配置）
3. **默认配置**（内置在代码中，生产环境使用）⭐ **新增**

---

## 自定义配置（可选）

如果你需要使用自己的 Supabase 项目，可以通过以下方式覆盖默认配置：
...
- ✅ **V251219.20+**：无需任何配置，开箱即用
- 🔧 **自定义配置**：使用上述方法覆盖默认配置
- 🔒 **安全性**：Anon Key 是公开的，所有安全由 RLS 保证
